...
function parseApiKeys(raw) { return String(raw||'').split(/[\s,|]+/).map(k=>k.trim()).filter(Boolean); }
...
//  LBI AUTH
// ══════════════════════════════════════════
async function getLbiPluginFromDB(opts = {}) {
    const db = await getCachedDB(opts.forceRefresh);
    if (!db || !Array.isArray(db.plugins)) {
        throw new Error('DB 접근 권한이 없습니다. 초기화 팝업에서 허용을 눌러주세요.');
    }
    const namePref = ((await Storage.get('studio_lbi_name')) || (await risuai.getArgument?.('lbi_plugin_name')) || '').trim();
    if (namePref) {
        const exact = db.plugins.find(p => p?.name === namePref);
        if (exact) return exact;
        const partial = db.plugins.find(p => p?.name?.includes(namePref));
        if (partial) return partial;
    }
    const auto = db.plugins.find(p => p?.name?.startsWith('LBI')) || db.plugins.find(p => p?.name?.toLowerCase().includes('lbi'));
    if (auto) { Logger.info('LBI 플러그인 자동 감지:', auto.name); return auto; }
    throw new Error('LBI 플러그인을 찾지 못했습니다. 설치 여부를 확인하세요.');
}

async function getLbiArgFromDB(key, opts = {}) {
    const plugin = await getLbiPluginFromDB(opts);
    const ra = plugin?.realArg ?? {};
    if (ra[key] != null) return ra[key];
    const tryAlias = (from, to, prefix) => {
        if (key.startsWith(from) && !key.startsWith(to)) {
            const alias = `${to}${key.slice(from.length)}`;
            if (ra[alias] != null) return ra[alias];
        }
        return undefined;
    };
    for (const [f,t] of [['common','common_'],['tools','tools_'],['common_','common'],['tools_','tools']]) {
        const v = tryAlias(f, t);
        if (v != null) return v;
    }
    return undefined;
}
...
async function getLbiArgCompat(...keys) {
    for (const key of keys) {
        const v = await getLbiArgFromDB(key).catch(() => undefined);
...
async function getCopilotApiToken(githubToken) {
    ...
    if (!res.ok || !res.data?.token) throw new Error('Copilot API 토큰 발급 실패');
    await Storage.set(CACHE_KEY, { token: res.data.token, expiresAt: Date.now() + 55 * 60 * 1000 });
...
async function loadPresetMsgs() {
    try { const r = await Storage.get(PRESET_MSGS_KEY); presetMsgs = r ? JSON.parse(r) : []; } catch(e) { presetMsgs = []; }
}
...
async function savePresetMsg() {
    ...
    try { await Storage.set(PRESET_MSGS_KEY, JSON.stringify(presetMsgs)); } catch(e) {}
    renderPresetMsgs();
...
function applyTheme(name) {
    ...
    Storage.set(THEME_STORAGE_KEY, JSON.stringify({ theme: name }));
}
function resetTheme() {
    document.body.className = document.body.className.replace(/\btheme-\S+/g, '').trim();
    document.documentElement.removeAttribute('style');
    Storage.set(THEME_STORAGE_KEY, JSON.stringify({}));
}
async function saveThemeSetting(key, value) {
    try {
        ...
        settings[key] = value;
        await Storage.set(THEME_STORAGE_KEY, JSON.stringify(settings));
    } catch(e) {}
}
